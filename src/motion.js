...
export class MotionSurface {
  ...
  scan() {
    ...
    // Scope to actual live assistant containers, never user/tool/editor text.
  ...
  async flight(submission) {
    ...
    orb.append(text); document.body.append(orb); this.nodes.add(orb);
    ...
    try {
      ...
      await this.animate(orb, [
        ...
        { transform: `translate(calc(-50% + ${(top.x-start.x)*.4}px),calc(-50% + ${(top.y-start.y)*.65}px)) scale(.85)`, offset: .5 },
        { transform: `translate(calc(-50% + ${top.x-start.x}px),calc(-50% + ${top.y-start.y}px)) scale(1)` },
      ], this.options.flight);
      orb.style.left = `${top.x}px`; orb.style.top = `${top.y}px`;
      if (!orb.isConnected || this.dead) return;
  ...
  dispose() {
    this.dead = true; this.observer.disconnect(); this.abort.abort();
...
export function decorateSidebar(active, enabled) {
