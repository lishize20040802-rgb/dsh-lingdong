...
function orbitRows(rows) {
  if (rows.length < 4) return rows;
  return Array.from({ length: 3 }, (_, index) => {
    const members = rows.filter((_2, position) => position % 3 === index);
    return {
      id: `group:${index}`,
      title: `任务组 ${index + 1} · ${members.length} 项`,
      state: members.some((row) => row.state === "working") ? "working" : members.every((row) => row.state === "completed") ? "completed" : "queued",
      statusText: members.map(agentLabel).join("；"),
      exiting: members.every((row) => row.exiting),
      members
    };
  });
}
...
/* Animate the host row's own background. No extra node, pseudo-element or stacking context. */
[data-row-key][role=treeitem].ld-row{background-image:linear-gradient(110deg,#85b9ed66 0%,#b098e973 20%,#e3adc866 40%,#84cdd366 60%,#a7b4ed73 80%,#85b9ed66 100%);background-size:350% 250%;background-repeat:no-repeat;animation:ld-row-native-flow 10s ease-in-out infinite}
[data-row-key][role=treeitem].ld-row-active:not(.ld-row-selected){background-image:linear-gradient(110deg,#68cbbb70 0%,#a8c5e966 20%,#e5b57870 40%,#a7d3b866 60%,#b8a2db66 80%,#68cbbb70 100%);animation-duration:7s}
[data-row-key][role=treeitem].ld-row-selected.ld-row-active{animation-duration:8s}
@keyframes ld-row-native-flow{0%,100%{background-position:0% 45%}25%{background-position:45% 100%}50%{background-position:100% 55%}75%{background-position:55% 0%}}
...
@media(prefers-reduced-motion:reduce){.ld-row,.ld-dock *,.ld-dock-line:after,.ld-ambient,.ld-stream:after{animation:none!important;transition:none!important}.ld-orb i{display:none}.ld-flight{display:none}}
@media(forced-colors:active){[data-row-key][role=treeitem].ld-row{background-image:none;animation:none}.ld-orb,.ld-stack{border:1px solid ButtonText;background:Canvas}.ld-stream:after{background:CanvasText}.ld-ambient,.ld-dock-line{display:none}}
...
.ld-orb:before{inset:-35%;background:radial-gradient(ellipse at 20% 25%,#69cbdac9,transparent 55%),radial-gradient(ellipse at 80% 35%,#9e8ce2c4,transparent 55%),radial-gradient(ellipse at 35% 85%,#edabc6bf,transparent 52%),radial-gradient(ellipse at 80% 85%,#91dab3b3,transparent 55%);animation:ld-pearl-colors 11s linear infinite;z-index:-1}
...
.ld-working i{width:4px;height:4px;left:5px;bottom:2px;border:1px solid #ffffffc9;background:#ffffff25;box-shadow:0 0 2px #ffffff33;animation:ld-live-bubble 3.2s ease-in-out infinite}
.ld-working i:nth-child(2){width:3px;height:3px;left:12px;animation-delay:-.64s}
.ld-working i:nth-child(3){width:5px;height:5px;left:8px;animation-delay:-1.28s}
.ld-working i:nth-child(4){width:3px;height:3px;left:15px;animation-delay:-1.92s}
.ld-working i:nth-child(5){width:4px;height:4px;left:3px;animation-delay:-2.56s}
@keyframes ld-pearl-colors{0%{transform:rotate(0deg) scale(1);opacity:.72}50%{transform:rotate(180deg) scale(1.12);opacity:1}100%{transform:rotate(360deg) scale(1);opacity:.72}}
@keyframes ld-live-bubble{0%{opacity:0;transform:translate(0,3px) scale(.45)}18%{opacity:.72}65%{opacity:.6;transform:translate(1px,-10px) scale(.85)}100%{opacity:0;transform:translate(-1px,-21px) scale(1.1)}}
...
.ld-paused,.ld-paused *,.ld-paused:before,.ld-paused:after,.ld-paused *:before,.ld-paused *:after{animation-play-state:paused!important}
...
/* A bounded constellation inside our header slot; host controls do not move. */
.ld-orbits-2,.ld-orbits-3{position:relative;display:block;flex:none;width:64px;height:44px;padding:0;overflow:visible;max-height:none}
.ld-orbits-2 .ld-orb-wrap,.ld-orbits-3 .ld-orb-wrap{position:absolute;left:50%;top:50%;width:18px;height:18px;margin:-9px 0 0 -9px}
.ld-orbits-2 .ld-orb,.ld-orbits-3 .ld-orb{width:18px;height:18px}
.ld-orbits-2 .ld-orb-wrap{animation:ld-binary-orbit 16s linear infinite}
.ld-orbits-2 .ld-orb-wrap:nth-child(2){animation-delay:-8s}
.ld-orbits-3 .ld-orb-wrap{animation:ld-three-body 21s linear infinite}
.ld-orbits-3 .ld-orb-wrap:nth-child(2){animation-delay:-7s}
.ld-orbits-3 .ld-orb-wrap:nth-child(3){animation-delay:-14s}
.ld-orbits-2:hover .ld-orb-wrap,.ld-orbits-3:hover .ld-orb-wrap,.ld-orbits-2:focus-within .ld-orb-wrap,.ld-orbits-3:focus-within .ld-orb-wrap{animation-play-state:paused}
@keyframes ld-binary-orbit{from{transform:rotate(0deg) translateX(14px) rotate(0deg)}to{transform:rotate(360deg) translateX(14px) rotate(-360deg)}}
@keyframes ld-three-body{0%{transform:rotate(0deg) translateX(17px) rotate(0deg) scale(1)}25%{transform:rotate(90deg) translateX(13px) rotate(-90deg) scale(.9)}50%{transform:rotate(180deg) translateX(18px) rotate(-180deg) scale(1.04)}75%{transform:rotate(270deg) translateX(13px) rotate(-270deg) scale(.92)}100%{transform:rotate(360deg) translateX(17px) rotate(-360deg) scale(1)}}
.ld-task-list{position:absolute;right:0;top:calc(100% + 10px);z-index:100;width:300px;max-width:80vw;max-height:260px;overflow:auto;padding:14px 16px;border:1px solid #91a9ba55;border-radius:16px;background:var(--dsw-alias-bg-base,#f7f9fa);color:var(--ld-ink);box-shadow:0 10px 32px #20304725;font-size:12px;line-height:1.7}
.ld-task-list ul{padding-left:17px;margin:8px 0 0}.ld-task-list li{margin:6px 0;overflow-wrap:anywhere}
@media(prefers-reduced-motion:reduce){.ld-orbits-2 .ld-orb-wrap,.ld-orbits-3 .ld-orb-wrap{animation:none!important}.ld-orbits-2 .ld-orb-wrap:first-child{transform:translateX(-12px)}.ld-orbits-2 .ld-orb-wrap:nth-child(2){transform:translateX(12px)}.ld-orbits-3 .ld-orb-wrap:first-child{transform:translate(0,-11px)}.ld-orbits-3 .ld-orb-wrap:nth-child(2){transform:translate(-13px,10px)}.ld-orbits-3 .ld-orb-wrap:nth-child(3){transform:translate(13px,10px)}}
`;
...
function apply(ctx) {
  ...
  function Dock(props) {
    ...
    const announced = (0, import_react.useRef)(/* @__PURE__ */ new Set());
    ...
    (0, import_react.useEffect)(() => {
      ...
      const elements2 = marker.current?.querySelectorAll(".ld-orb");
      for (const index of destinations) if (elements2?.[index]) surface?.emitTask(elements2[index]);
    ...
    return h(
      ...
      options.enabled && h(
        "div",
        { id: orbsId, className: `ld-orbs ld-orbits-${displayed.length}`, "aria-label": "后台任务与子代理状态" },
        displayed.map((row) => h(Orb, { key: row.id, row, reduced, root, onDone: remove }))
      ),
      options.enabled && rows.length >= 4 && h("button", {
        className: "ld-stack",
        type: "button",
        onClick: () => setExpanded(!expanded),
        "aria-controls": `${orbsId}-tasks`,
        "aria-label": expanded ? "收起任务列表" : `查看全部 ${rows.length} 个任务`,
        "aria-expanded": expanded
      }, `${rows.length} 项`),
      options.enabled && expanded && rows.length >= 4 && h(
        "div",
        { id: `${orbsId}-tasks`, className: "ld-task-list", role: "region", "aria-label": "全部任务状态" },
